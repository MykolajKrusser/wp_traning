...
function mk_events_post_type()
{
    $event_type = 'events';
    $labels = array(
        'name' => __('Events', 'Post Type General Name', 'mk'),
        'singular_name' => __('Event', 'Post Type Singular Name', 'mk'),
        ...
        'search_items' => __('Search Event', 'mk'),
    ...
    register_taxonomy('events_tags',array($event_type), array(
        'hierarchical' => true,
        'labels' => $labels,
        'show_ui' => true,
        'show_in_rest' => true,
...
add_action('init', 'mk_events_post_type');


function insert_or_update($import_data)
{
    ...
    foreach ($import_data as $key => $value) {
        ...
        if ($existed_data)
            $existed_data_id = $existed_data[$key]->ID;

        $import_data_update = array(
            'ID' => $existed_data_id,
            'post_title' => $value->title,
            'post_content'  => $value->about,
            'post_type' => 'events',
            'post_status' => 'publish'
        );

        $unique_id = (int)get_field('unique_id', $existed_data_id);

        if ($unique_id == 0){
            wp_insert_post($import_data_update);
            update_field('unique_id', $value->id, $existed_data_id);
            update_field('organizer', $value->organizer, $existed_data_id);
            update_field('timestamp', $value->timestamp, $existed_data_id);
            update_field('email', $value->email, $existed_data_id);
            update_field('address', $value->address, $existed_data_id);
            update_field('latitude', $value->latitude, $existed_data_id);
            update_field('longitude', $value->longitude, $existed_data_id);
            wp_set_object_terms( $existed_data_id, $value->tags, 'events_tags' );

        }else{
            wp_update_post($import_data_update);
            update_field('unique_id', $value->id, $existed_data_id);
            update_field('organizer', $value->organizer, $existed_data_id);
            update_field('timestamp', $value->timestamp, $existed_data_id);
            update_field('email', $value->email, $existed_data_id);
...
add_action( 'init', 'mk_import_events' );
